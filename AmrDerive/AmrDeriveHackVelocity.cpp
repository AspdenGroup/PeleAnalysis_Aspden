...

{
    ...
    */
    ...
    // Out suffix
    char fSizeStr[16];
    sprintf(fSizeStr,"_hack%02i",fSize);
	...
	Real timer_start = ParallelDescriptor::second();
        ...
        for (int iLevel=0; iLevel<nLevels; iLevel++) {
            ...
            mf.set(iLevel, new MultiFab(amrData.boxArray(iLevel), nVars, nGrow0));
	...
	// Make the multifab to hold the output data
        MultiFab mfOut;
	for (int iLevel=0; iLevel<nLevels; iLevel++) {
	  mfOut.define(amrData.boxArray(iLevel), nVars, nGrow0, Fab_allocate);
	  mfOut.setVal(iLevel);
	  for(MFIter mfi(mf[iLevel]); mfi.isValid(); ++mfi) {
	    ...
	    FORT_HACKVELOCITY(myData,ARLIM(dlo),ARLIM(dhi),
