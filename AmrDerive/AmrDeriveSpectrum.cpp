...
int main (int argc, char* argv[])
{
    ...
    if (nVars==0)
	BoxLib::Abort("Must specify vars to load");
    if (ParallelDescriptor::IOProcessor())
    ...
    if (div_free && nVars!=3) 
      BoxLib::Abort("Must specify three vars (assumed to be x y z velocities) if using div_free");
    ...
    if (div_free && density_weighting) 
      BoxLib::Abort("Density weighting and div free at the same time doesn't make sense (yet)");
    ...
    Array<int> destFills(nVars+density_weighting);
...
*/
    ...
    Box probDomain(amrData.ProbDomain()[finestLevel]);
      ...
      // Set BoxLib and FourierTranform array sizes
	...
	Qz[iVar]=(Real*)malloc(sizeof(Real)*Qkx);    for (int k=0; k<Qkx; k++)    Qz[iVar][k] = 0.0;
      ...
      int ngrow(0);
      MultiFab mf;
      mf.define(domainBoxArray, nVars+density_weighting, ngrow, domainDistMap, Fab_allocate);
...
//     }
  // ========
	...
	std::cout << "   ...done (" << timer_stop-timer_start << "s)." << std::endl;
      ...
      //
      for (int iVar=0; iVar<nVars; iVar++) {
	local_data[iVar]   = (fftw_real*)    malloc(sizeof(fftw_real) * total_local_size);   if (local_data[iVar] == NULL) BoxLib::Abort("Malloc fail (local_data)");
...
void plan_ffts(Box &probDomain, BoxArray &domainBoxArray, Array<int> &pmap)
{
    ...
    if (local_ix==0)
      BoxLib::Abort("Number of processors doesn't divide domain size (see usage note)");
...
void filter(MultiFab &mf)
{
  ...
  Array<fftw_real*>    filtered_data(nFiltVars);
  Array<fftw_complex*> filtered_data_c(nFiltVars);
  for (int iVar=0; iVar<nFiltVars; iVar++) {
    filtered_data[iVar]   = (fftw_real*)    malloc(sizeof(fftw_real) * total_local_size);   if (filtered_data[iVar] == NULL) BoxLib::Abort("Malloc fail (filtered_data)");
  ...
  int ngrow(0);
  MultiFab mfOut;
  mfOut.define(mf.boxArray(), nOutVars, ngrow, mf.DistributionMap(), Fab_allocate);
