      ...
      char* argv[])
{
	...
	for(MFIter ntmfi(mf); ntmfi.isValid(); ++ntmfi) {
		...
		Real H2O_val = varPtr[0][cell];
		std::cout << "ArraySize = " << sizeof(varPtr[0]) << "/" << ix*jx - 1 << std::endl;
		//std::cout << "Cell:" << cell << "/" << ix*jx - 1  << std::endl;
		//std::cout << "H2O_val = " << H2O_val << std::endl;
		if (H2O_val > 0.001)
		  {
    ...
    if (ParallelDescriptor::IOProcessor()) {
	FILE *file = fopen("FlameVolume.dat","w");
	for (int iPlot=0; iPlot<nPlotFiles; iPlot++)
	    fprintf(file,"%e %e\n",time[iPlot],vol_val[iPlot]);
	fclose(file);
    }
