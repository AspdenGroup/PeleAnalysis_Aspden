...
void integrateTheta(int dirx, int diry, int dirz, Vector<Vector<Vector<Real>>>& outdata, Vector<Real>& r, Vector<Real>& z, AmrData& amrData, Vector<MultiFab*> indata, int nVars, int finestLevel, int cComp, Real cMin, Real cMax, int avg) {
		...
		if (ridx < ldirr) {
		  for (int rx = 0; rx < refRatio; rx++) {
		    for (int ry = 0; ry < refRatio; ry++) {
		      outdata[0][refRatio*ridx+rx][refRatio*k+ry] += dzLev/((Real)bandingFactor*bandingFactor);
		      for (int n = 1; n < nVars+1; n++) {
			outdata[n][refRatio*ridx+rx][refRatio*k+ry] += dzLev*inbox(i,j,k,n-1)/((Real)bandingFactor*bandingFactor);
		      }
		    }
		  }
		}
  ...
  for (int i = 0; i < ldirr; i++) {
    for (int n = 0; n<nVars+1; n++) {
      ParallelDescriptor::ReduceRealSum(outdata[n][i].data(),ldirz);
    }
  }
  if (avg) {
    for (int n = 1; n<nVars+1; n++) {
      for (int i = 0; i < ldirr; i++) {
	for (int j = 0; j < ldirz; j++) {
...
int main(int argc, char *argv[])
{
  amrex::Initialize(argc, argv);
  {
    ...
    std::string outfile= infile+"_integral_rz";
    if(!cVar.empty()) {
    ...
    //do 1d integration
    integrateTheta(dirx,diry,dirz,outdata,r,z,amrData,indata,nVars,finestLevel,cComp,cMin,cMax,avg);
	...
	writeDat2D(outdata[0],outfile+"_thetaLength.dat",ldirr,ldirz);
	for (int n = 1; n < nVars+1; n++) {
	...
	writePPM(outdata[0],outfile+"_thetaLength.ppm",ldirr,ldirz,goPastMax,vMin[0],vMax[0]);
	for (int n = 1; n < nVars+1; n++) { 
	  writePPM(outdata[n],outfile+"_"+vars[n-1]+".ppm",ldirr,ldirz,goPastMax,vMin[n],vMax[n]);
	}
