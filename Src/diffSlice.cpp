...
int main(int argc, char *argv[])
{
  ...
  for (MFIter mfi(*mf); mfi.isValid(); ++mfi) {
    ...
    amrex::Print() << "infab box = " << fab.box() << std::endl;
    amrex::Print() << "top box = " << topFab.box() << std::endl;
    amrex::Print() << "diff box = " << diffFab.box() << std::endl;
    if (iFile == slicenum) {
      topFab.copy(fab); //hold onto this fab for the minute
    }    
    diffFab.copy(topFab);
    diffFab.minus(fab);
