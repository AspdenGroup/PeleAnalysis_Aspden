      ...
      char* argv[])
{
    ...
    int lev = 0;
    pp.query("level",level);
    // file box array
    //BoxArray fileba  = amrData.probDomain()[level];
    ...
    fileData[lev] = new MultiFab(newba[lev],dm[lev],nComp,0);
    // load data
    amrData.FillVar(*fileData[lev],level,amrData.PlotVarNames(),comps);
    //}
    ...
    WriteSingleLevelPlotfile(outfile,*dat[lev],names,geoms[lev],amrData.Time(),levelSteps[lev]);
