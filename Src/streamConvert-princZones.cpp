      ...
      char* argv[])
{
    ...
    {
	std::cerr << names[iComp] << std::endl;
	std::string compVar = names[iComp];
	std::string K1name = "MeanCurvature_prog_H2";
	std::string K2name = "GaussianCurvature_prog_H2";
	if (K1name.compare(compVar) == 0)
	    mkComp = iComp;
	if (K2name.compare(compVar) == 0)
	    gkComp = iComp;
    }
    if (mkComp == -1 || gkComp == -1) 
	Abort("No Mean or Gaussian Curvature found!");
    
    std::cerr << "Zone" << std::endl;
    std::string outFileName=infile+"/Zone.dat";
    FILE *outfile=fopen(outFileName.c_str(),"w");
    // loop over each node
    for (int iNode=0; iNode<nNodes; iNode++)
    {
	...
	for (int iPt=0; iPt<nPts; iPt++, loc += amrex::BASISV(1))
	{
	    if (iPt == (nPts + 1) / 2)
	    {
		Real mK = fab(loc,mkComp);
		Real gK = fab(loc,gkComp);
		Real k1 = mK + sqrt(fabs(pow(mK,2) - gK));
		Real k2 = mK - sqrt(fabs(pow(mK,2) - gK));
		int Zone;
		if (sqrt(pow(k1,2) + pow(k2,2)) * ls < range)
		    Zone = 0;
		else if (k2 > k1 / 2)
		    Zone = 1;
		else if (fabs(k2 / k1) < 0.5)
		    Zone = 2;
		else if (fabs(k1 / k2) < 0.5)
		    Zone = 3;
		else if (k1 < k2 / 2)
		    Zone = 4;
		else
