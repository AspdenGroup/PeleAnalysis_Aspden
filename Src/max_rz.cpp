...
void maxTheta(int dirx, int diry, int dirz, Vector<Vector<Vector<Real>>>& outdata, Vector<Real>& r, Vector<Real>& z, AmrData& amrData, Vector<MultiFab*> indata, int nVars, int finestLevel, int cComp, Real cMin, Real cMax) {
  ...
  for (int lev = finestLevel; lev >= 0; lev--) {
    ...
    Print() << "Max-ing on level "<< lev << std::endl;
	  ...
	  if (inbox(i,j,k,nVars) > 1e-8 && (cComp < 0 || (inbox(i,j,k,cComp) >= cMin && inbox(i,j,k,cComp) < cMax))) {	    
		...
		for (int ry = 0; ry < refRatio; ry++) {
		  for (int n = 0; n < nVars; n++) {
		    outdata[n][refRatio*ridx+rx][refRatio*k+ry] = std::max(outdata[n][refRatio*ridx+rx][refRatio*k+ry],inbox(i,j,k,n));
		  }
		}
	      }
	    }
  ...
  for (int i = 0; i < ldirr; i++) {
    for (int n = 0; n<nVars; n++) {
      ParallelDescriptor::ReduceRealMax(outdata[n][i].data(),ldirz);
    }
  }
...
int main(int argc, char *argv[])
{
  amrex::Initialize(argc, argv);
  {
    ...
    std::string outfile= infile+"_max_rz";
    if(!cVar.empty()) {
    ...
    Vector<Real> r(ldirr);
    Vector<Real> z(ldirz);
    Vector<Real> tmp1(ldirz,1e-10);
    Vector<Vector<Real>> tmp2(ldirr,tmp1);
    Vector<Vector<Vector<Real>>> outdata(nVars,tmp2);
    //do min
    maxTheta(dirx,diry,dirz,outdata,r,z,amrData,indata,nVars,finestLevel,cComp,cMin,cMax);
    Print() << "Maximum check completed" << std::endl;
