      ...
      char* argv[])
{
      ...
      for (int i=0; i<nElts; ++i)
	{
	  ...
	  if (MYLEN == 2) {
	    ...
	    Real c1 = nodeData[offsetData1*nComp+cComp];
	    Real c2 = nodeData[offsetData2*nComp+cComp];
	    Real c3 = nodeData[offsetData3*nComp+cComp];
	    Real x1 = nodeData[offsetData1*nComp]; //x coord of node 1
	    Real x2 = nodeData[offsetData2*nComp]; //x coord of node 2
	    ...
	    if (c1 > cMin && c2 > cMin && c3 > cMin && c1 < cMax && c2 < cMax && c3 < cMax) {
	      totArea += eltArea;
	    }
	  }
	}
