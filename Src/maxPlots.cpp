      ...
      char* argv[])
{
    ...
    std::string outfile = "maxPlot";
    pp.query("outfile",outfile);
    
    DataServices::SetBatchMode();
    Amrvis::FileType fileType(Amrvis::NEWPLT);

    Print() << "Setting up maxPlotfile..." << std::endl;
    ...
    Print() << "... creating maxMF" << std::endl;
    DistributionMapping dm(ba);
    MultiFab maxMF(ba,dm,nComp,0);
    Print() << "... maxMF created" << std::endl;
    ...
    Real timeFinal = 0;
    Print() << "maxPlotfile setup completed." << std::endl;
    ...
    Print() << "Starting taking maximum... " << std::endl;
	  ...
	  AMREX_PARALLEL_FOR_3D ( bx, i, j, k,
				  {
					...
					outbox(i,j,k,n) = std::max(inbox(i,j,k,n),outbox(i,j,k,n));
				      }							 
				    }
    ...
    Print() << "Completed finding maxes." << std::endl;
    Print() << "Writing " << outfile << "..." << std::endl;
    WriteSingleLevelPlotfile(outfile,maxMF,names,geoms,timeFinal-time0,levelSteps);
    }
    amrex::Finalize();
    return 0;
}
