...
{
  std::cerr << "usage:\n";
  std::cerr << argv[0] << " infiles=<> outfile=<> comps=<>" ;
  exit(1);
}
...
int main (int argc, char* argv[])
{
  ...
  int ncomps(pp.countval("comps"));
  int id_comp_last = 0;
  Vector<std::string> comps; comps.resize(ncomps);
  pp.getarr("comps",comps);  
  Vector<std::string> new_comps = comps;
  ...
  for (int lev=0; lev<Nlev; ++lev)
    {
      ...
      fileData[lev] = new MultiFab(amrData0.boxArray(lev),dm,ncomps,0);
    }
  ...
  for (int iFile = 0; iFile < nfiles; iFile++) {
    ...
    for (int i = 0; i < plotVarNames.size(); ++i) {  // loop though current pltfile variable names
      for (int j = 0; j < new_comps.size(); ++j) {   // loop though remaining search variable names
	if (plotVarNames[i] == new_comps[j]) {       // if search name = current pltfile variable name
	  idcomp.push_back(i);                       // sets index location of plotVarName that matches
	  new_comps.erase(new_comps.begin() + j);    // delets search variable name so we do not get repeats
	}
      }
    }
	...
	for (int i=0; i<idcomp.size(); ++i) {
	  fileData[lev]->copy(amrData.GetGrids(lev,idcomp[i]),0,id_comp_last+i,1);	    
	}
      }
  ...
  if (new_comps.size() > 0) {
    ...
    for (int i = 0; i < new_comps.size(); ++i)
      Print() << new_comps[i] << std::endl;
    DataServices::Dispatch(DataServices::ExitRequest, NULL);	      
  }
