      ...
      char* argv[])
{
    ...
    const int nComp = 2 + intComps.size() 
    ...
    int nCompOut = nExtra+nInt+nAvg+nKn+nAux+nGrad+2*nPeak+nPFCR;
    ...
    int oFirstKn = oFirstAvg + nAvg;
    int oFirstAux = oFirstKn + nKn;
    ...
    // Find the mean and gaussian curvature index
    int Mcurvloc = -1;
    int Gcurvloc = -1;
    for (int i=0; i<avgComps.size(); ++i)
    {
        outNames[oFirstAvg+i] = names[namescnt++] + std::string("_avg");
	if (outNames[oFirstAvg+i] == "MeanCurvature_prog_H2_avg")
	    Mcurvloc = oFirstAvg+i;
	else if (outNames[oFirstAvg+i] == "GaussianCurvature_prog_H2_avg")
	    Gcurvloc = oFirstAvg+i;
    }
    if (Mcurvloc == -1 || Gcurvloc == -1)
	Abort("Gaussian or Mean curvature not given");
    ...
    // Set name for principle curvatures
    outNames[oFirstKn] = std::string("princCurvature1_avg");
    outNames[oFirstKn+1] = std::string("princeCurvature2_avg");
	...
	volInt[oFirstKn] = volInt[Mcurvloc] + sqrt(sqrt(pow(pow(volInt[Mcurvloc],2) - volInt[Gcurvloc],2))); // k1
	volInt[oFirstKn+1] = volInt[Mcurvloc] - sqrt(sqrt(pow(pow(volInt[Mcurvloc],2) - volInt[Gcurvloc],2))); // k2
